...
// side effect (`readFileStats`), its typed failure, and the FileSystem
...
import { readFileStats } from "@joel-stack/core";
...
// FileSystem requirement of readFileStats surfaces in the actor's type.
const readStats = fromEffect({
  effect: ({ input }) => readFileStats(input.path),
