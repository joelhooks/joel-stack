...
- `rat-stack-patterns` (`scripts/oxlint-plugin-patterns.ts`): `contract-binding-matches-name` (`const searchContract = defineContract("search", …)`), `no-module-level-mutable-state` (module `let`/`var` in `apps/*/src` and `packages/*/src` leaks across Worker requests), `acquire-release-constructs-in-acquire-body` (build pools inside acquire, not before), and `no-empty-contract-input` (`Schema.Struct({})` breaks MCP; give the input one optional field).
